...
In the near future, MatchMass will be hosted at Tallinn University of Technology domain (taltech.ee) at the address [matchmass.taltech.ee](https://matchmass.taltech.ee/). However, you can test its earilier version as a **locally running browser version** made in [stlite](https://edit.share.stlite.net/?sampleAppId=intro). The app code and data are encoded into the URL as a hash like `https://share.stlite.net/#!ChBz...`. Probably due to the excessive length of the link I was not able to make a functional hyperlink here in GitHub. Links generated by URL shorteners did not work either. Therefore, please find *stlite_browser_version.md* file and copy the full link (ctrl+a) from there to a new tab in Firefox (Chrome loads only "Hello World" page).
